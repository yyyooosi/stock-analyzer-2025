 ...
 * トレンドを判定（過去3ヶ月の平均と比較）
 */
function determineTrend(observations: FREDSeriesObservation[]): 'rising' | 'falling' | 'stable' {
  ...
  if (validObs.length < 4) return 'stable';

  const latest = validObs[0];
  const previous = validObs.slice(1, 4).reduce((sum, val) => sum + val, 0) / 3;
